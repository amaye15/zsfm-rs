...
use config::LagLlamaConfig;
use convert::{convert, ConvertOptions};
use download::download_model;
use gguf::GGMLType;
use infer::LagLlamaModel;

#[derive(Parser)]
#[command(name = "lag-llama-rs", about = "Download, convert, and run Lag-Llama time-series forecasting")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    ...
    /// Run Lag-Llama forecasting from a GGUF file.
    ///
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Command::Infer { gguf } => {
            ...
            println!("{}", forecast_json("lag-llama", total_ctx, horizon, fc_choices)?);
        }
    }
