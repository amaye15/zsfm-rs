...
impl LagLlama {
    ...
    pub fn new(gguf: &str) -> PyResult<Self> {
        ...
        let model = LagLlamaModel::load(std::path::Path::new(gguf), config)
    ...
    pub fn forecast(
        ...
        to_py(py, &build_json("lag-llama", total_ctx, horizon, &fc_choices))
    }
}

pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<LagLlama>()?;
    Ok(())
}
