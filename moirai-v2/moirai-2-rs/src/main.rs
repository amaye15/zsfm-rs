...
use config::Moirai2Config;
use convert::{convert, ConvertOptions};
use download::download_model;
use gguf::GGMLType;
use infer::Moirai2Model;

#[derive(Parser)]
#[command(name = "moirai-2-rs", about = "Download, convert, and run Moirai-2.0-R-small time-series forecasting")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    ...
    /// Run Moirai-2.0-R-small forecasting from a GGUF file.
    ///
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Command::Infer { gguf } => {
            ...
            println!("{}", forecast_json("moirai-2", total_ctx, horizon, fc_outputs)?);
        }
    }
