...
impl MomentModel {
    ...
    pub fn forecast(&self, context: &[f32], horizon: usize) -> Result<Vec<f32>> {
        ...
        h = rms_norm(&h, &self.norm_f_w, cfg.layer_norm_eps)?;

        // Apply head to future positions
        let future_h = h.narrow(0, ctx_patches, n_fc)?; // [n_fc, 1024]
        let pred = linear_bias(&future_h, &self.head_w, &self.head_b)?; // [n_fc, 8]
    ...
    fn forward_block(
        ...
        let res = hidden;
        let h = rms_norm(hidden, &blk.attn_norm_w, self.config.layer_norm_eps)?;
        let h = self.t5_self_attn(&h, blk, rel_bias, seq_len)?;
        let h = (h + res)?;

        // Pre-norm FFN (with residual)
        let res2 = h.clone();
        let h2 = rms_norm(&h, &blk.ffn_norm_w, self.config.layer_norm_eps)?;
...
/// T5 LayerNorm: RMSNorm with configurable eps.
...
fn gated_gelu_ffn(
    ...
    let gate  = linear_nobias(x, wi0_w)?.gelu_erf()?;
    let value = linear_nobias(x, wi1_w)?;
    let h = (gate * value)?;
    linear_nobias(&h, wo_w)
}
