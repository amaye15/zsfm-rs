...
use config::MomentConfig;
use convert::{convert, ConvertOptions};
use download::download_model;
use gguf::GGMLType;
use infer::MomentModel;

#[derive(Parser)]
#[command(name = "moment-rs", about = "Download, convert, and run MOMENT-1-large time-series forecasting")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    ...
    /// Run MOMENT forecasting from a GGUF file.
    ///
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Command::Infer { gguf } => {
            ...
            println!("{}", forecast_json("moment", total_ctx, horizon, fc_choices)?);
        }
    }
