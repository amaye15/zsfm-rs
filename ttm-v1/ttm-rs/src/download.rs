    ...
    let shards =
        match fetch_file(&client, repo_id, "model.safetensors.index.json", model_dir).await {
            ...
            Err(_) => {
                ...
                let shard = fetch_file(&client, repo_id, "model.safetensors", model_dir).await?;
                vec![shard]
            }
        };

    Ok(ModelFiles { config_json, safetensors_shards: shards })
}

fn build_client(hf_token: Option<&str>) -> anyhow::Result<reqwest::Client> {
    let mut headers = reqwest::header::HeaderMap::new();
    headers.insert(reqwest::header::USER_AGENT, "ttm-rs/0.1".parse().unwrap());
