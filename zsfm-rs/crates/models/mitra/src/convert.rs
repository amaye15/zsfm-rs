...
fn write_metadata(writer: &mut GGUFWriter, config: &MitraConfig) {
    writer.add_metadata("general.architecture", GGUFMetaValue::String("mitra".into()));
    writer.add_metadata(
        "general.name",
        GGUFMetaValue::String(
            match config.task {
                Task::Classification => "autogluon/mitra-classifier",
                Task::Regression => "autogluon/mitra-regressor",
            }
            .into(),
        ),
    );
    ...
    writer.add_metadata("mitra.dim", GGUFMetaValue::Uint32(config.dim as u32));
    writer.add_metadata("mitra.n_layers", GGUFMetaValue::Uint32(config.n_layers as u32));
    writer.add_metadata("mitra.n_heads", GGUFMetaValue::Uint32(config.n_heads as u32));
    writer.add_metadata("mitra.dim_output", GGUFMetaValue::Uint32(config.dim_output as u32));
}
