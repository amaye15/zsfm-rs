...
impl MoiraiModel {
    ...
    pub fn forecast(&self, context: &[f32], horizon: usize) -> Result<Vec<f32>> {
        ...
        h = zsfm_nn::rms_norm(&h, Some(&self.norm_f_w), 1e-6)?;
    ...
    fn forward_block(
        ...
        let res = hidden;
        let h = zsfm_nn::rms_norm(hidden, Some(&blk.norm1_w), 1e-6)?;
        let h = self.qk_attn(&h, blk, is_masked, seq_len)?;
        let h = (h + res)?;

        let res2 = h.clone();
        let h2 = zsfm_nn::rms_norm(&h, Some(&blk.norm2_w), 1e-6)?;
        let h2 = zsfm_nn::swiglu_ffn(&h2, &blk.ffn_fc1_w, &blk.ffn_fc2_w, &blk.ffn_gate_w)?;
        Ok((h2 + res2)?)
    }
