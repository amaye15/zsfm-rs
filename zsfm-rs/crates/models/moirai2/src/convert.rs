...
fn write_metadata(writer: &mut GGUFWriter, config: &Moirai2Config) {
    writer.add_metadata("general.architecture",      GGUFMetaValue::String("moirai2".into()));
    writer.add_metadata("general.name",              GGUFMetaValue::String("Moirai-2.0-R-small".into()));
    writer.add_metadata("moirai2.d_model",           GGUFMetaValue::Uint32(config.d_model as u32));
    writer.add_metadata("moirai2.n_layers",          GGUFMetaValue::Uint32(config.n_layers as u32));
    writer.add_metadata("moirai2.n_heads",           GGUFMetaValue::Uint32(config.n_heads as u32));
    writer.add_metadata("moirai2.head_dim",          GGUFMetaValue::Uint32(config.head_dim as u32));
    writer.add_metadata("moirai2.d_ff",              GGUFMetaValue::Uint32(config.d_ff as u32));
    writer.add_metadata("moirai2.patch_size",        GGUFMetaValue::Uint32(config.patch_size as u32));
    writer.add_metadata("moirai2.num_predict_token", GGUFMetaValue::Uint32(config.num_predict_token as u32));
    writer.add_metadata("moirai2.num_quantiles",     GGUFMetaValue::Uint32(config.num_quantiles as u32));
    writer.add_metadata("moirai2.max_seq_len",       GGUFMetaValue::Uint32(config.max_seq_len as u32));
    writer.add_metadata("moirai2.rope_dim",          GGUFMetaValue::Uint32(config.rope_dim as u32));
}
