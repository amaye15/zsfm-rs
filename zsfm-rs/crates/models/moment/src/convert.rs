...
fn write_metadata(writer: &mut GGUFWriter, config: &MomentConfig) {
    writer.add_metadata("general.architecture",    GGUFMetaValue::String("moment".into()));
    writer.add_metadata("general.name",            GGUFMetaValue::String("MOMENT-1-large".into()));
    writer.add_metadata("moment.d_model",          GGUFMetaValue::Uint32(config.d_model as u32));
    writer.add_metadata("moment.n_layers",         GGUFMetaValue::Uint32(config.n_layers as u32));
    writer.add_metadata("moment.n_heads",          GGUFMetaValue::Uint32(config.n_heads as u32));
    writer.add_metadata("moment.head_dim",         GGUFMetaValue::Uint32(config.head_dim as u32));
    writer.add_metadata("moment.d_ff",             GGUFMetaValue::Uint32(config.d_ff as u32));
    writer.add_metadata("moment.seq_len",          GGUFMetaValue::Uint32(config.seq_len as u32));
    writer.add_metadata("moment.patch_len",        GGUFMetaValue::Uint32(config.patch_len as u32));
    writer.add_metadata("moment.patch_stride",     GGUFMetaValue::Uint32(config.patch_stride as u32));
    writer.add_metadata("moment.num_patches",      GGUFMetaValue::Uint32(config.num_patches as u32));
    writer.add_metadata("moment.layer_norm_eps",   GGUFMetaValue::Float64(config.layer_norm_eps));
}
...
fn quantize_q8_0(values: &[f32]) -> anyhow::Result<Vec<u8>> {
    ...
    if values.len() % BLOCK != 0 {
        anyhow::bail!("Q8_0 requires count divisible by {BLOCK}");
    }
